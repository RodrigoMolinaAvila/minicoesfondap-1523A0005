...
# Definir las URLs base para Serie Informe
base_url = "https://lyd.org/serie-informe/page/"
page_limit = 12  # Número máximo de páginas a recorrer
...
for page in range(1, page_limit + 1):
    ...
    if response.status_code == 200:
        ...
        for article in articles:
            ...
            try:
                # Etiquetas del estudio (si existen)
                etiquetas = ', '.join([tag.text.strip() for tag in article.select('span.elementor-post-info__terms-list-item')])
            except AttributeError:
                etiquetas = 'Sin etiquetas'  # Si no hay etiquetas, se asigna 'Sin etiquetas'

            # Solo agregar a la lista si la fecha, título y enlace tienen información válida
            if all([fecha, titulo, enlace]):
                data.append({
                    'Fecha': fecha,
                    'Título': titulo,
                    'Etiquetas': etiquetas,
...
output_path = os.path.join(output_dir, 'lyd_serie_informe.csv')
